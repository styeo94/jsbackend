...
##### \_\_2.2.1 Node.js의 구성요소

##### \_\_2.2.2 자바스크립트 실행을 위한 V8 엔진

##### \_\_2.2.3 이벤트 루프와 운영체제 단 비동기 API 및

> 스레드 풀을 지원하는 libuv

##### \_\_2.2.4 Node.js 아키텍처

#### \_2.3 Node.js의 기술적인 특징

##### \_\_2.3.1 싱글 스레드

##### \_\_2.3.2 이벤트 기반 아키텍처
...
##### \_\_2.5.1 hello.js 파일 생성 및 실행

##### \_\_2.5.2 curl 내려받기 및 테스트해보기
...
- 참고사항
  ...
  _ https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/splice \* https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/reduce
  > 크롬 확장 프로그램 JSON Viewer 를 설치하면, JSON 포맷된 형태로 브라우저에서 편하게 볼 수 있음.

#### \_3.7 익스프레스로 간단한 API 서버 만들기

##### \_\_3.7.1 게시판 API 코드 작성하기

##### \_\_3.7.2 API 규칙에 맞게 서버 구현하기

#### \_3.8 게시판 API 테스트하기

##### \_\_3.8.1 curl로 GET 호출하기

##### \_\_3.8.2 curl로 POST를 호출해 게시글 등록하기

##### \_\_3.8.3 curl로 DELETE를 호출해 게시글 삭제하기
...
##### \_\_5.3.1 동시에 여러 Promise 객체 호출하기

- Promise.all() 안에 배열로 호출함수 넣음.

##### \_\_5.3.2 Promise 예외 처리하기
...
##### \_\_6.5.2 몽구스로 스키마 만들기

##### \_\_6.5.3 몽구스와 익스프레스로 CRUD API 만들기
...
##### \_\_7.4.3 상세페이지 UI 만들기

#### \_7.5 API 만들기

##### \_\_7.5.1 몽고디비 연결을 위한 유틸리티 만들기
...
##### \_\_7.5.2 UI 페이지에서 사용할 핸들바 커스텀 헬퍼 만들기
...
##### \_\_7.5.4 글쓰기 API 만들기

##### \_\_7.5.5 리스트 API 만들기
...
##### \_\_8.3.3 NestJS의 모듈과 컨트롤러 만들기

##### \_\_8.3.4 hello-nest 앱 실행시켜보기
...
##### \_\_8.4.3 블로그 API 작성하기

##### \_\_8.4.4 메모리에 데이터를 저장하는 API 만들기

```
npm run start:dev
```

##### \_\_8.4.5 파일에 정보를 저장하도록 API 업그레이드하기
...
##### \_\_8.6.3 몽고디비를 사용하는 리포지토리 추가하기

##### \_\_8.6.4 서비스 코드 변경

##### \_\_8.6.5 모듈에 몽고 디비 설정과 프로바이더 설정 추가하기
...
##### \_\_9.3.1 app.module.ts에 ConfigModule 설정하기
...
##### \_\_9.3.3 app.controller.ts에 코드 추가하기
...
##### \_\_9.4.3 날씨 API 테스트용 핸들러 함수로 테스트하기
...
##### \_\_9.5.1 환경별로 서버가 기동되도록 스크립트 수정하기
...
##### \_\_9.5.2 local, dev, prod 환경 변수 생성
...
##### \_\_9.5.3 환경 변수에 따라서 다른 환경 변수 파일을 사용하도록 설정 수정하기
...
##### \_\_9.6.2 ConfigModule에 load 옵션 추가하기
...
##### \_\_9.10.1 확장 변수를 사용할 수 있게 추가 설정하기
...
> 지금까지 테스트 해 오면서 .http 에서 '###' 의 기능을 몰랐다. 단순히 주석인줄 알았는데, 이게 있어야. send request 버튼이 생성되는 것. 빠르게 테스트를 진행해 봤다. 어느정도 손이 따라가는 게 신기하다. import는 구지비 넣지 않아도 하단에서 사용을 하면 자동으로 삽입이 되는 것도 지금에서야 알게됐다.

#### \_10.3 파이프로 유효성 검증하기

- 지금까지는 구현을 위해 숨가쁘게 달려왔지만, 사용자 입력값에 대한 유효성 검증은 필수!
- NestJS는 파이프pipe를 사용해서 유효성 검증 처리.
- 댜양한 파이프들이 있고, 직접 만들 수도 있지만, 여기서는 가장 간편한 ValidationPipe를 이용.
- class-transformer 는 JSON 정보를 클래스 객체로 변경. 받은 요청 payload을 변환한 클래스가 컨트롤러의 핸들러 메서드의 매개변수에 선언되어 있는 클래스와 같다면 유효성 검증을 실시. class-validator는 데코레이터를 사용해 간편하게 유효성 검증을 하는 라이브러리.

##### \_\_10.3.1 전역 ValidationPipe 설정하기

- 유효성 검증을 하려면 ValidationPipe를 main.ts에 설정해야.
- ValidationPipe 추가 방법
  1. 의존성을 설치
  2. 임포트
  3. 전역 파이프 설정에 ValidationPipe 객체를 생성해 넘음.

```
npm install class-validator class-transformer
```

##### \_\_10.3.2 UserDto 만들기

##### \_\_10.3.3 테스트하기

> 좋다! 영문인 것만 빼면! 한글로 출력되도록 하면 더할나위 없겠음. 사실, 우리같은 백엔드 개발자들 FrontEnd까지는 욕심일지 모를 일이다.

#### \_10.4 인증 모듈 생성 및 회원 가입하기

- 인증의 두 측면
  - 정확성측면 : 사용자의 자격증명을 기존 정보를 기반으로 확인 후 인증 토큰 발급
  - 시간 측명 : 사용자에게 부여된 인증 토큰을 특정 기간 동안만 유효하게 하는 것
- 인증을 만드는 2가지 방법 : 쿠키와 토큰

##### \_\_10.4.1 인증 모듈 만들기 및 설정하기
...
> --no-spec은 테스트파일을 만들지 않겠다는 의미.

##### \_\_10.4.2 회원 가입 메서드 만들기

- 패스워드 암호화를 위해 bcrypt 설치
...
##### \_\_10.4.3 sqlite 익스텐션으로 테이블 확인하기 (p.399)
...
##### \_\_10.5.1 AuthService에 이메일과 패스워드 검증 로직 만들기
...
##### \_\_10.6.2 로그인과 인증에 사용할 가드 구현하기

- 여기서는 로그인에 사용가드와 인증후에 사용할 가드로 나눠서 생성 (auth.guard.ts)

##### \_\_10.6.3 세션에 정보를 저장하고 읽는 세션 시리얼라이저 구현하기
...
##### \_\_10.6.4 email, password 인증 로직이 있는 LocalStrategy 파일 작성하기

- 인증 유형별 다양한 스트래티지가 존재함.

##### \_\_10.6.5 auth.module.ts에 설정 추가하기

##### \_\_10.6.6 테스트하기

> 일단, 테스트까지 가긴 했는데, 세션 생성이 안된 것 같다. 어딘가 잘못된 것 같은데, 확인이 필요하다.

##### \_\_10.6.7 로그인과 세션 저장까지 순서
...
##### \_\_11.1.1 OAuth 프로토콜 흐름

##### \_\_11.1.2 액세스 토큰을 재발행하는 흐름

#### \_11.2 구글 OAuth를 사용하기 위한 준비하기

##### \_\_11.2.1 구글 클라우드에서 프로젝트 생성하기
...
##### \_\_11.2.2 OAuth 동의 화면을 만들기

##### \_\_11.2.3 OAuth 클라이언트의 ID와 비밀번호 만들기
...
##### \_\_13.2.3 클라이언트 측 구현하기 : client.html 파일 작성

##### \_\_13.2.4 테스트하기

##### \_\_13.2.5 향후 과제 확인하기

#### \_13.3 간단한 채팅 애플리케이션 만들기 : socket.io

##### \_\_13.3.1 socket.io 프로젝트 생성하기
...
##### \_\_13.3.6 클라이언트를 위한 index.html 수정하기

##### \_\_13.3.7 테스트하기

#### \_13.4 채팅방 기능이 있는 채팅 애플리케이션 만들기

##### \_\_13.4.1 네임스페이스 사용하기

> 뭔가 이미 알고는 있던 단어인데 NestJS 안에서 새롭게 정의된 것이라고 해야하나? 아님, 나만 모르고 다 아는 사실이었던 걸까? 네임스페이스가 여기서는 새롭게 정의된다. 이게 Node.js 세계관에선 비일비재한가 봄.

- chat.gateway.ts 에 아래처럼 데코레이터 추가.

```
@WebSocketGateway({ namespace: 'chat' })
```

- 그리고 기존 자바스립트 부분에서 네임스페이스를 변경함.

```
const socket = io('http://localhost:3000'); //변경전
const socket = io('http://localhost:3000/chat'); //변경후
```

> 아니, 근데.. 뒤로 갈수록 오타가 대환장 파티네..
...
##### \_\_13.4.4 공지 영역과 채팅방 입장 구현하기

##### \_\_13.4.5 채팅방에서 대화 나누기 구현하기
